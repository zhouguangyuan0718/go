...
func TestLLVMValueIsTombstone(t *testing.T) {
	tests := []struct {
		name string
		v    *Value
		want bool
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			if got := llvmValueIsTombstone(test.v); got != test.want {
				t.Fatalf("llvmValueIsTombstone(%s, uses=%d) = %t, want %t", test.v.Op, test.v.Uses, got, test.want)
			}
		})
	}
}

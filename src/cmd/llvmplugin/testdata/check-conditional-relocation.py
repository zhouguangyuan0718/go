...
FUNCTION = "conditional_safepoint"
...
def main():
    ...
    if ranges[1]["start"] != queries[0]["call_offset"] - 1:
        fail("safepoint stack-map range does not start at the CALL")
    ...
    print(
        ...
        f"locals bits {sorted(safepoint_bits)} -> {sorted(morestack_bits)}"
    )
...
if __name__ == "__main__":
    try:
        ...
        print(f"check-conditional-relocation: {error}", file=sys.stderr)
        sys.exit(1)

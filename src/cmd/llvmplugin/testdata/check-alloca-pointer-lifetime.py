...
def main():
    ...
    if locals_body.count("call void @llvm.lifetime.") != 2:
        fail("locals-only lifetime markers were not preserved")
    ...
    if stack_body.count("call void @llvm.lifetime.") != 2:
        fail("stack-object lifetime markers were not preserved")
    ...
    initialize = stack_body.find("store ptr null")
    lifetime_start = stack_body.find("call void @llvm.lifetime.start")
    active = stack_body.find('"gc-live"(ptr %slot)')
    if min(initialize, lifetime_start, active) < 0 or not lifetime_start < initialize < active:
        fail("stack object is not initialized at its lifetime start")
    if args.objview:
